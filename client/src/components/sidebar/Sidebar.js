    ...
    <div className={`Sidebar${layout === "max" ? "" : " short"}`}>
      <ul className="Sidebar-list">
        ...
        <li
          id="Profile"
          className="row"
          onClick={() => history.push(`/profile/${login.user.username}`)}
        >
        ...
        <li id="Post" className="row">
          ...
          >
            {layout === "max" ? (
              ...
              <MdPostAdd id="icon" size="1.8em"></MdPostAdd>
            )}
          </button>
        </li>
      </ul>
      {toggleNewPost ? (
        <Popup onClose={setToggleNewPost} Component={newPostElem} />
      ) : (
        <></>
      )}
